...
- **启动入口路径去耦** — `tinda.sh`、`tinda.bat`、`quick.sh`、`quick.bat` 不再依赖固定父目录名；项目内入口从脚本所在目录推导项目根，PATH 安装器只生成相对引用或在无法相对定位时明确失败。
- **WorkGraph Actor 层级图** — Actor Map 改为自上而下的层级图，父子关系使用灰色正交折线连接，减少横向树状布局的拥挤感。
- **WorkGraph 选中态降噪** — 节点点击后只保留相关节点与相关边的灰白高亮，其余节点和连线降噪；再次点击节点或点击画布空白可清除选中态。
- **WorkGraph 节点日志窗** — 左键节点会打开以节点中心为锚点的日志浮窗，窗口内置节点设置按钮并预留当前节点设置界面骨架。
- **WorkGraph 文档补录** — 新增 `TindaAgent/docs/WORKGRAPH.md`，记录当前 WorkGraph 数据模型、三视图布局、hover/选中交互、状态灯、视觉语言、节点日志窗口、设置界面预留和已知限制。

## v1.12.14 - 2026-05-31

### Fixed

- **Lane Timeline 泳道语义修正** — Lane Timeline 不再按状态排序角色，改为按事件首次出现顺序排列泳道，恢复工作流阅读顺序。
- **Lane 节点裁切修复** — Lane 视图使用独立节点尺寸和更紧凑的泳道高度，8 条样例泳道可在首屏完整显示且中文摘要不再被截断。
- **Lane 连线降噪** — Lane 视图的因果线改为低存在感折线，避免跨泳道大曲线干扰节点阅读。
- **详情面板折叠** — Lane Timeline 默认折叠底部 JSON 详情区，仅保留选中摘要和展开按钮，把纵向空间让给泳道图。

## v1.12.13 - 2026-05-31

### Changed

- **WorkGraph 首屏可读性优化** — 根据 Windows 实机截图调整 WorkGraph 布局：减少底部详情区高度，压缩左侧栏宽度，扩大画布有效宽度。
- **时间线自适应布局** — Event Timeline 和 Lane Timeline 会按当前画布宽度计算事件间距，小规模工作流默认完整落在首屏内，减少横向拖拽。
- **节点文本渲染优化** — 事件节点标题改为稳定事件 ID，摘要支持两行展示，避免 `e_00...` 和中文摘要被过早截断。
- **时间轴视觉引导** — Event Timeline 新增 `TIME ORDER` 轴线，用一条低亮度时间线补足大画布中的方向感。
...
- **日志列表加载加速** — `/logs/files` 不再对 `audit_events` 做 `COUNT(*)` 全表统计，改用最新事件行生成概览，并对旧日志文件列表做短周期缓存。
- **大日志尾读限流** — legacy 文件尾读增加字节上限，避免 `llm_request.jsonl` 等超长 JSONL 为凑行数一次加载数十 MB。
- **按 ID 查询加速** — 旧日志搜索新增 SQLite 范围索引和命中缓存，查询前按 `min_id/max_id` 剪枝，命中旧文件后反向分块扫描并缓存结果。
...
- **Web 设置优先级** — 聊天页启动时以服务端 `web-settings.json` 为最高优先级加载配置，浏览器 `localStorage` 仅作为离线兜底，避免旧缓存覆盖已保存设置。
- **快捷按钮顺序保真** — `/web-settings` 保存和回读会规范化快捷按钮数组，保留用户设置顺序并去除重复/未知项；设置页渲染也按存储顺序展示已启用按钮。
- **设置测试隔离** — HTTP 功能烟测会备份并恢复真实 `web-settings.json`，避免测试污染本机运行配置。

## v1.12.6 - 2026-05-30

### Fixed

- **思考内容渲染顺序** — assistant 气泡持久化时按模型真实 `history_delta` 重建模块，确保 `thinking` 在回复正文前渲染。
- **旧反序记录自动修复** — 加载历史 JSON/SQLite 会话时自动纠正已保存为“正文在前、思考在后”的 assistant 内容模块。
- **流式显示顺序兜底** — 实时流渲染的 reasoning 聚合显示改为思考在前、正文在后，刷新前后顺序保持一致。

## v1.12.5 - 2026-05-30

### Fixed

- **会话记录时间顺序保真** — 消息 JSON 分区与 SQLite 镜像写入时统一按 `created_at`/兼容时间字段升序重排，并重新编号为 `"1"`, `"2"`，避免历史 key 顺序与真实时间顺序不一致。
- **气泡内部模块序号化** — 每条气泡的独立内容模块继续按旧 Python JSON 结构保存为连续数字键，修复乱序 content key 导致的刷新渲染顺序漂移。
- **压缩摘要渲染顺序** — 前端渲染优先使用服务端 effective `seq`，保留压缩摘要的逻辑位置，同时对缺少 `seq` 的临时数据使用时间戳兜底。
...
- **会话历史占位清理** — 流式生成中的“正在生成”占位不再写入最终会话消息；历史数据加载时会自动剔除这类临时消息，避免重载后把占位当正文渲染。
- **会话 JSON 分区回写** — 会话元数据、消息、终端、计划和配置继续按原 Python JSON 分区写回对应目录，同时保留 SQLite 镜像用于索引与兼容。
...
2. 修复多运行时根分叉并完成数据迁移：新增跨根迁移脚本，将项目盘符下的 `.tinda/agent` 合并迁移到 WSL 主目录并生成迁移报告。
