...
因为 AI 辅助写代码引入的 bug 有点多，导致这一版反复修了好几次才稳定。我发誓下次应该更仔细地规划再动手。目前大部分 bug 已修复。

1. 新增终端执行工具 `run_terminal`：LLM 可直接执行 shell 命令，权限要求 63（PUBLIC+TOOL），系统操作额外校验 SYSTEM_EXECUTE。
2. 终端安全策略模块：命令白名单/黑名单、22 种系统操作自动检测、bypass 确认模式（仅管理员）。
3. 终端确认气泡：前端聊天页内联确认卡片，允许/拒绝按钮，折叠展开，不进 LLM 上下文，持久化存储。
4. 设置页终端控制面板：白名单/黑名单输入、bypass 开关。
5. 移除假工具调用检测，误报实在太多了。
6. 修了一堆 AI 搞出来的低级 bug：token 索引写错了属性名、_caller_perm 给所有工具硬塞了个不认识的参数导致全线 500、缓存函数自己递归调用自己爆栈、忘了 import os。
