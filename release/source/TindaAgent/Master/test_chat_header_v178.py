...
"""
    ...
    def test_chat_renderer_has_plan_tool_marker_view(self) -> None:
        ...
        self.assertIn("已完成", self.renderer_content)
        self.assertNotIn("待用户确认完成", self.renderer_content)
    ...
    def test_chat_has_movable_plan_panel_ui(self) -> None:
        ...
        self.assertIn("let planCurrentBySession = new Map();", self.content)
        self.assertIn("const currentPlan = sid ? planCurrentBySession.get(sid) : null;", self.content)
        self.assertIn("renderPlanFloat(currentPlan);", self.content)
        self.assertIn('sid !== String(getSessionId() || "").trim()', self.content)
        ...
        self.assertNotIn("requires_completion_confirmation", self.content)
        self.assertNotIn("completion_confirmation_state", self.content)
        ...
        self.assertNotIn("待用户确认完成", self.content)
        self.assertNotIn("已确认完成", self.content)
        self.assertIn("已完成", self.content)
